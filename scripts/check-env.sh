...
if ! command -v docker >/dev/null 2>&1; then
  WARNINGS+=("Docker isn't installed. The background 'Intelligence' stack (Postgres/Redis) will be unavailable.")
elif ! docker info >/dev/null 2>&1; then
  WARNINGS+=("Docker isn't running. The background 'Intelligence' stack (Postgres/Redis) will be unavailable.")
fi
...
if [[ ! -f "$AGENT_ENV" ]]; then
  ...
  for VAR in GEMINI_API_KEY JIRA_URL JIRA_EMAIL JIRA_API_TOKEN; do
    ...
    if is_stub "$val"; then
      case "$VAR" in
        GEMINI_API_KEY)
          ...
          ;;
        JIRA_URL)
          PROBLEMS+=("$VAR is unset (or a stub) in apps/agent/.env. Example: https://your-domain.atlassian.net")
          ;;
        JIRA_EMAIL)
          PROBLEMS+=("$VAR is unset in apps/agent/.env. The email address for your Atlassian account.")
          ;;
        JIRA_API_TOKEN)
          PROBLEMS+=("$VAR is unset in apps/agent/.env. Get one at: https://id.atlassian.com/manage-profile/security/api-tokens")
          ;;
      esac
    fi
  done
...
if [[ ${#PROBLEMS[@]} -eq 0 ]]; then
  HEALTH_OUT="$(cd "$REPO_ROOT/apps/agent" && uv run python -m src.jira_tools --check 2>&1 || true)"
  if ! grep -q "^OK: " <<<"$HEALTH_OUT"; then
    PROBLEMS+=("Jira health check failed:
$HEALTH_OUT")
  fi
fi
